...
export function oauth(http) {
  this.codeVerifier = crypto.randomBytes(32).toString('hex');
  this.OAuthAppId = process.env.OAUTH_APP_ID || '***REMOVED***';
  ...
  this.getOAuthURL = function (redirectUrl, oauthScope) {
  ...
  this.getAccessToken = function (host, redirectUrl, code) {
  ...
  this.refreshToken = function(host, redirectUrl, refreshToken) {
    return new Promise((resolve, reject) => {
      const headers = { 'Content-Type': 'application/x-www-form-urlencoded' };
      const payload = {
  ...
  this.oauthLogout = async function () {
   ...
   */
  this.getOauthAppAuthorization = async function () {
    const headers = {
  ...
  this.revokeOauthAppAuthorization = async function (authorizationId) {
  ...
  // check expiry
  this.compareOAuthExpiry = async function (force = false) {
