...
export function Entry (http, data) {
  ...
  if (data && data.entry) {
    ...
    /**
       * @description Metadata is a piece of information that lets you describe or classify an asset/entry.
       * @param {String} uid The UID of the MetaData you want to get details.
       * @returns {MetaData} Instance of MetaData.
       * @example
       * import * as contentstack from '@contentstack/management'
       * const client = contentstack.client()
       *
       ...
       */
    this.metadata = (metadata_uid = null) => {
      const data = { stackHeaders: this.stackHeaders }
      data.content_type_uid = this.content_type_uid
      data.type = "entry"
      data.entity_uid = this.uid
      if (metadata_uid) {
        data.metadata = { uid: metadata_uid,  }
      }
      return new MetaData(http, data)
    }
  } else {
    /**
     * @description The Create an entry call creates a new entry for the selected content type.
