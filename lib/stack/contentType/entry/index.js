...
export function Entry (http, data = {}) {
  ...
  this.urlPath = `conteent_types/${data.content_type.uid}/entries`

  if (data && data.entry) {
    Object.assign(this, cloneDeep(data.entry))
    this.urlPath = `conteent_types/${data.content_type.uid}/entries/${this.uid}`
    /**
     ...
     *
     */
    this.update = update(http, 'entries')
   ...
   * const client = contentstack.client({})
   *
   * client.stack().contentType('content_type_uid').entry('uid').create({name: 'My New contentType'})
   * .then((entry) => console.log(entry))
   */
   ...
   * const client = contentstack.client({})
   *
   * client.stack().contentType().query({name: 'My New Stack'})
...
export function EntryCollection (http, data) {
  ...
  const entryCollection = obj.map((userdata) => {
    return new Entry(http, { entry: userdata, content_type: 'uid', stackHeaders: data.stackHeaders })
  })
