...
import { ContentType, ContentTypeCollection } from './contentType/index'
...
export function Stack (http, data) {
   ...
   * .then((contentType) => console.log(contentType))
   */
  stack.contentType = function (contentTypeUid = null) {
    if (contentTypeUid == null) {
      return entity(http, 'content_types', stackHeaders.headers, ContentType, ContentTypeCollection)
    }
    return ContentType(http, { content_type: {
      uid: contentTypeUid
    },
    stackHeaders: stackHeaders.headers })
  }
  ...
  stack.role = function (uid = null) {
    if (!uid) {
      return entity({ http: http, urlPath: '/roles', stackHeaders: stackHeaders.headers, wrapper: Role, wrapperCollection: RoleCollection })
    }
