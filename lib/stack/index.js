...
export function Stack (http, data) {
  ...
  if (data) {
     ...
     *
     * client.stack('api_key').contenglobalFieldtType('globalField_uid').fetch()
     * .then((globalField) => console.log(globalField))
     */
    ...
    this.transferOwnership = async (email) => {
      try {
        const response = await http.post(`${this.urlPath}/transfer_ownership`, { transfer_to: email }, this.stackHeaders)
        if (response.data) {
    ...
    this.settings = async () => {
      try {
        const response = await http.get(`${this.urlPath}/settings`, this.stackHeaders)
        if (response.data) {
    ...
    this.resetSettings = async () => {
      try {
        const response = await http.post(`${this.urlPath}/settings`, { stack_settings: { discrete_variables: {}, stack_variables: {} } }, this.stackHeaders)
        if (response.data) {
          return response.data.stack_settings
        } else {
          return error(response)
    ...
    this.addSettings = async (stackVariables = {}) => {
      try {
        const response = await http.post(`${this.urlPath}/settings`, { stack_settings: { stack_variables: stackVariables } }, this.stackHeaders)
        if (response.data) {
          return response.data.stack_settings
        } else {
          return error(response)
    ...
    this.share = async (emails = [], roles = {}) => {
      try {
        const response = await http.post(`${this.urlPath}/share`, { emails: emails, roles: roles }, this.stackHeaders)
        if (response.data) {
          return response.data.notice
        } else {
          return error(response)
    ...
    this.unShare = async (email) => {
      try {
        const response = await http.post(`${this.urlPath}/unshare`, { email: email }, this.stackHeaders)
        if (response.data) {
          return response.data.notice
        } else {
          return error(response)
   ...
   * .then((stack) => console.log(stack))
   */
    this.create = create({ http: http, params: null })
   ...
   * @returns {Array<Stack>} Array of Stack.
   *
   * @example
   * import * as contentstack from 'contentstack'
   * const client = contentstack.client({})
