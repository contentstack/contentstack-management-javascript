...
export function DeliveryToken (http, data = {}) {
  ...
  if (data.tokens) {
    Object.assign(this, cloneDeep(data.tokens))
     ...
     *
     */
    this.update = update(http, 'tokens')
...
export function DeliveryTokenCollection (http, data) {
  ...
  const deliveryTokenCollection = obj.map((userdata) => {
    return new DeliveryToken(http, { tokens: userdata, stackHeaders: data.stackHeaders })
  })
