...
export function Webhook (http, data = {}) {
  ...
  if (data.webhook) {
     ...
     * .then((webhook) => console.log(webhook))
     */
    this.executions = async function (param) {
      const headers = {
        headers: {
     ...
     */
    this.retry = async function (executionUid) {
      const headers = {
        headers: {
          ...cloneDeep(this.stackHeaders)
        }
    ...
    /**
     * @description The Query on Webhook will allow to fetch details of all or specific Webhook
     * @memberof Webhook
     * @func query
     ...
     * @returns {Array<Webhook>} Array of ContentTyoe.
     *
     * @example
     * import * as contentstack from 'contentstack'
     * const client = contentstack.client({})
     *
     * client.stack().webhook().query({name: 'My New Stack'})
     * .then((webhooks) => console.log(webhooks))
     */
    this.query = query({ http: http, wrapperCollection: WebhookCollection })
  }
