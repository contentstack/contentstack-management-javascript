...
describe('Contentstack Branch test', () => {
  ...
  it('Branch MergeQueue find test', done => {
    ...
    makeBranch()
      ...
      .then((response) => {
        checkBranch(response)
        expect(response.merge_details).to.eql(branchMergeQueueFindMock.merge_details)
        expect(response.errors).to.eql(branchMergeQueueFindMock.errors)
        done()
      })
      .catch(done)
  })
  ...
  it('Branch MergeQueue fetch test', done => {
    var mock = new MockAdapter(axios)
    mock.onGet('/stacks/branches/merge-queue').reply(200, branchMergeQueueFetchMock)
    makeBranch()
      .mergeQueue('UID')
      .fetch()
      .then((response) => {
