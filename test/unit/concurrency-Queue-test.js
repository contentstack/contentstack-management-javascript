...
describe('Concurrency queue test', () => {
    ...
    it('Initialization with custom config negative value test', done => {
        try {
            ...
            expect(error.message).to.be.equal('Concurrency Manager Error: minimun concurrent requests is 1')
            done()
        }
    })

    it('Initialization with custom config retry limit nagative value test', done => {
        try {
            makeConcurrencyQueue({retryLimit: -10})
            expect.fail('Negative retry limit should fail')
        } catch (error) {
            expect(error.message).to.be.equal('Retry Policy Error: minimun retry limit is 1')
            done()
        }
    })
    ...
    it('Initialization with custom config retry delay value test', done => {
        try {
            ...
            expect(error.message).to.be.equal('Retry Policy Error: minimun retry delay for requests is 300')
            done()
        }
    })
