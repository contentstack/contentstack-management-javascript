...
describe('Contentstack HTTP Client', () => {
  ...
  it('Contentstack Http Client X-User-Agent Header', done => {
    ...
    createClientRewireApi.__Rewire__('contentstackClient', sinon.stub().returns({}))
    client(axios, {})
    expect(createHttpClientStub.args[0][1].defaultHostName).to.be.equal('api.contentstack.io', 'Default host name not match')
    expect(createHttpClientStub.args[0][1].headers['X-User-Agent']).to.be.equal(SDK, 'Default host name not match')
    createClientRewireApi.__ResetDependency__('httpClient')
    createClientRewireApi.__ResetDependency__('contentstackClient')
    done()
  })
  ...
  it('Contentstack Http Client Default Host Custom', done => {
    ...
    createClientRewireApi.__Rewire__('httpClient', createHttpClientStub)
    createClientRewireApi.__Rewire__('contentstackClient', sinon.stub().returns({}))
    client(axios, { defaultHostName: 'contentstack.com' })
    expect(createHttpClientStub.args[0][1].defaultHostName).to.be.equal('contentstack.com', 'Default host name not match')
    createClientRewireApi.__ResetDependency__('httpClient')
    createClientRewireApi.__ResetDependency__('contentstackClient')
    done()
  })
  ...
  it('Contentstack Http Client User-Agent Header', done => {
    ...
    createClientRewireApi.__Rewire__('contentstackClient', sinon.stub().returns({}))
    client(axios, {})
    const headerParts = createHttpClientStub.args[0][1].headers['User-Agent'].split('; ')
    expect(headerParts[0]).to.be.match(/^sdk @contentstack\/contentstack-management-javascript\/.+/, 'User agent sdk does not match')
  ...
  it('Contentstack Http Client User-Agent Header Custom', done => {
    ...
    createClientRewireApi.__Rewire__('httpClient', createHttpClientStub)
    createClientRewireApi.__Rewire__('contentstackClient', sinon.stub().returns({}))
    client(axios, { application: 'myApplication/1.0.0', integration: 'myIntegration/1.2.0', feature: 'feature' })
    const headerParts = createHttpClientStub.args[0][1].headers['User-Agent'].split('; ')
    ...
    expect(headerParts[3]).to.be.match(/^sdk @contentstack\/contentstack-management-javascript\/.+/, 'User agent sdk does not match')
    createClientRewireApi.__ResetDependency__('httpClient')
    createClientRewireApi.__ResetDependency__('contentstackClient')
    done()
  })
