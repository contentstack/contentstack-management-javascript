...
describe('Taxonomy API Tests', () => {
  ...
  describe('Taxonomy Publishing', () => {
    ...
    it('should unpublish taxonomy items from an environment', async function () {
      this.timeout(30000)
      const taxonomyUid = testData.taxonomies?.category?.uid
      if (!taxonomyUid) {
        this.skip()
      ...
      try {
        const response = await stack.taxonomy().unpublish(unpublishData)
        expect(response).to.be.an('object')
      } catch (e) {
        expect(e.status).to.be.oneOf([400, 403, 404, 412, 422])
      }
    ...
    it('should unpublish taxonomy with api_version parameter', async function () {
      this.timeout(30000)
      const taxonomyUid = testData.taxonomies?.category?.uid
      if (!taxonomyUid) {
        this.skip()
      ...
      try {
        const response = await stack.taxonomy().unpublish(unpublishData, '3.2')
        expect(response).to.be.an('object')
      } catch (e) {
        expect(e.status).to.be.oneOf([400, 403, 404, 412, 422])
      }
    ...
    it('should fail unpublish with empty items', async function () {
      this.timeout(15000)
      try {
