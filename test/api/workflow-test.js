...
describe('Workflow api Test', () => {
  ...
  it('Create Publish rules', done => {
    const publishingRule = cloneDeep(firstPublishRules)
    publishingRule.environment = environment[0].uid
    publishingRule.approvers = {
      users: [user.uid],
      roles: [roles[0].uid]
    }
    makeWorkflow()
      ...
      .then(publishRule => {
        ...
        expect(publishRule.content_types.length).to.be.equal(publishingRule.content_types.length)
        expect(publishRule.content_types[0]).to.be.equal(publishingRule.content_types[0])
  ...
  it('Create second Publish rules', done => {
    const publishingRule = cloneDeep(secondPublishRules)
    publishingRule.environment = environment[1].uid
    publishingRule.approvers = {
      users: [user.uid],
      roles: [roles[1].uid]
    }
  ...
  it('Create Single page Publish rules', done => {
    const publishingRule = cloneDeep(secondPublishRules)
    publishingRule.content_types = ['single_page']
    publishingRule.environment = environment[1].uid
    publishingRule.approvers = {
      users: [user.uid],
      roles: [roles[1].uid]
